...
A.public_get_store = async (env, body) => {
  ...
  return {
    ...
    hero_kicker: set?.hero_kicker || "", hero_heading: set?.hero_heading || "",
    hero_subheading: set?.hero_subheading || "", hero_cta_text: set?.hero_cta_text || "",
    hero_images: p(set?.hero_images, []),
...
A.store_login = async (env, body, ctx) => {
  ...
  return { store: s, settings: set ? { ...set, contact: p(set.contact, []), hero_images: p(set.hero_images, []) } : null };
};
  ...
  if (pr.product_id) {
    const r = await env.DB.prepare(`UPDATE products SET name=?, price=?, stock=?, category_id=?, description=?,
      ...
      .bind(pr.name.trim(), Number(pr.price), Number(pr.stock) || 0, pr.category_id, pr.description || "",
        j(images), j(pr.variant_groups || []), pr.is_active === 0 ? 0 : 1, pr.product_id, s.store_id).run();
...
A.store_save_settings = async (env, body, ctx) => {
  ...
  const heroImagesIn = Array.isArray(st.hero_images) ? st.hero_images.filter(Boolean).slice(0, 8) : [];
  const heroImages = [];
  for (const im of heroImagesIn) heroImages.push(im.startsWith("data:") ? await putFile(env, im, "hero") : im);

  const stmts = [env.DB.prepare(`UPDATE store_settings SET logo_file_id=?, accent_color=?, announcement=?, tagline=?, contact=?,
    hero_kicker=?, hero_heading=?, hero_subheading=?, hero_cta_text=?, hero_images=?
    WHERE store_id=?`).bind(logo, st.accent_color || "#173d24", st.announcement || "", st.tagline || "", j(st.contact || []),
      st.hero_kicker || "", st.hero_heading || "", st.hero_subheading || "", st.hero_cta_text || "", j(heroImages),
      s.store_id)];
