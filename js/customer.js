...
(function () {
  ...
  function renderHero() {
    ...
    if (photos.length) {
      const { gallery, dots } = buildGallery(photos, S.store.business_name, 3000);
  ...
  function buildGallery(photos, altBase, autoplayMs) {
    ...
    if (autoplayMs && photos.length > 1) wireAutoplay(gallery, photos.length, autoplayMs);
    return { gallery, dots };
  }
  ...
  function wireAutoplay(gallery, count, ms) {
    if (window.matchMedia?.("(prefers-reduced-motion: reduce)").matches) return;
    let timer = null;
    const advance = () => {
      const next = (Math.round(gallery.scrollLeft / gallery.clientWidth) + 1) % count;
  ...
  function openProduct(p) {
    ...
    const { gallery, dots } = photos.length
      ...
      : { gallery: el("div", { class: "gallery" }, el("div", { class: "gal-slide" }, el("div", { class: "ph" }, "\u{1F4E6}"))), dots: null };
