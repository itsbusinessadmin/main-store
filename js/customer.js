...
(function () {
  ...
  async function boot() {
    ...
    try {
      // getStore() and the first page of products don't actually depend on
      // each other — both only need PUBLIC_ID, which is already known. Firing
      // them together instead of one-after-another roughly halves the time
      // to first paint on a fresh visit.
  ...
  function paintHeader() {
    ...
    if (logoSrc) { splashLogo.src = logoSrc; splashLogo.classList.remove("hide"); }
  ...
  function buildGallery(photos, altBase, autoplayMs) {
    const gallery = el("div", { class: "gallery" }, ...photos.map(id => {
      ...
      /* The photo is shown uncropped, so a portrait or square shot leaves gaps
         ...
         only, hence aria-hidden. */
      return el("div", { class: "gal-slide" },
        el("div", { class: "gal-bg", "aria-hidden": "true", style: `background-image:url("${photo.src.replace(/"/g, "%22")}")` }),
        photo);
    }));

    /* Measure against the real slide positions rather than assuming each slide
       is exactly one clientWidth across. The flex gap means slide N actually
       sits at N * (width + gap), so the naive division drifts further out of
       step with every photo and eventually lights the wrong dot. */
    const slides = () => Array.from(gallery.children);
    const offsetOf = i => {
      const s = slides();
      if (!s.length) return 0;
      const target = s[Math.max(0, Math.min(s.length - 1, i))];
      return target.offsetLeft - s[0].offsetLeft;
    };
    const currentIndex = () => {
      const s = slides();
      if (!s.length) return 0;
      ...
      s.forEach((_, i) => {
        const d = Math.abs(offsetOf(i) - x);
        if (d < bestDist) { bestDist = d; best = i; }
      });
      return best;
    };
    const goTo = (i, smooth = true) => gallery.scrollTo({
      left: offsetOf(i), behavior: smooth ? "smooth" : "auto"
    });
    ...
    /* Arrows are for pointer users; touch users swipe. CSS hides them on
       coarse-pointer devices, so they cost nothing on phones and tablets. */
    const arrow = (dir, glyph, label) => el("button", {
    ...
    const sync = () => {
      ...
      if (dots) $$("i", dots).forEach((d, j) => d.classList.toggle("on", i === j));
      /* Dim rather than disable at the ends, so focus is never yanked away
         mid-interaction from a button the person is still using. */
      if (prev) prev.classList.toggle("off", i <= 0);
      if (next) next.classList.toggle("off", i >= photos.length - 1);
    };
    if (dots || prev) {
      gallery.addEventListener("scroll", debounce(sync, 60));
      sync();
    }

    const view = el("div", { class: "gal-view" }, gallery, prev, next);
    if (autoplayMs && photos.length > 1) wireAutoplay(gallery, photos.length, goTo, currentIndex, autoplayMs);
    return { gallery, dots, view };
  }

  /* Auto-advance a gallery every `ms`, looping back to the start. Pauses
     while the tab is hidden or the person is actively touching/dragging it,
     and is skipped entirely under prefers-reduced-motion since
     auto-advancing content is a real accessibility concern (WCAG 2.2.2). */
  function wireAutoplay(gallery, count, goTo, currentIndex, ms) {
    if (window.matchMedia?.("(prefers-reduced-motion: reduce)").matches) return;
    let timer = null;
    const advance = () => goTo((currentIndex() + 1) % count);
    const start = () => { stop(); timer = setInterval(advance, ms); };
    const stop = () => { if (timer) { clearInterval(timer); timer = null; } };
    let resumeT;
    const pauseThenResume = () => { stop(); clearTimeout(resumeT); resumeT = setTimeout(start, ms + 1500); };
    gallery.addEventListener("pointerdown", pauseThenResume, { passive: true });
  ...
  async function loadProducts(reset = false) {
    ...
    S.loading = true;
    const host = $("#products");
    /* boot() now fires this alongside getStore() rather than after it, so it's
       possible getStore() fails and fatal() replaces #app (removing #products)
       while this request is still in flight. Bail quietly instead of throwing
       on a host that no longer exists. */
    if (!host) { S.loading = false; return; }
  ...
  function priceDisplay(p) {
    ...
    if (!range || range.min === range.max) return money(p.price);
  ...
  function productCard(p) {
    ...
    return el("button", { class: "p-card", onclick: () => openProduct(p) },
      ...
      el("div", { class: "body" },
        ...
        el("div", { class: "price" }, priceDisplay(p))),
  ...
  function openProduct(p) {
    ...
    // Before the customer has clicked anything, preview the cheapest possible total
    // (so the price shown isn't a misleading ₱0.00). The moment they click any option,
    // switch to strict mode: the price only ever reflects what's actually been picked,
    // never a phantom minimum for a group they haven't touched yet.
    let touched = false;
    const livePrice = () => (p.variant_groups || []).reduce((price, g) => {
      ...
      if (touched || g.required === false) return price;
      const deltas = g.price_delta?.length ? g.price_delta : [0];
