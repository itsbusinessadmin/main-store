...
(function () {
  ...
  function openProduct(p) {
    ...
    // While a required group hasn't been picked yet, preview using its cheapest option
    // so the price shown always reflects the lowest price the customer can actually pay
    // (e.g. shows the "Small" price before anything is clicked).
    const livePrice = () => (p.variant_groups || []).reduce((price, g) => {
      ...
      if (g.required === false) return price;
      const deltas = g.price_delta?.length ? g.price_delta : [0];
